...
class Tree
{
    ...
    function getLists(){
        $back = $this->getChildFull(0);
        return $back;
    }
    function getChildFull($id){
        $return = array();
        $list = $this->getChild($id);
        if( !empty($list) ){
            foreach( $list as $k=>$v ){
                $return[$k] = $this->data[$v];
                $return[$k]['sons'] = $this->getChildFull($v);
            }
        }
        return $return;
    }
    ...
    {
        return $space ? str_repeat($space, $this->layer[$id]-1) : $this->layer[$id];
    }
